/*
  Show ASCII leters on LED matrix 8x8
 ...
 * Arduino UNO
 ...
 v 0.1.0 - Martin Mikala, May 1, 2016
...
void loop(){ 
  ...
  for(int c = 32;c < 128; c++) {                            // show characters 32 to 127
    for(int d = 0; d < 100; d++) {                          // redraw char 100 times
      for(int i = 0; i < 8; i++){                           // loop for each column
        digitalWrite(columns[i], colon);                    // ON col i
        byte col = font[c][revbit[i]] << 3;                 //read bits for row, shift to left by 3   
        for(int j = 0; j < 8; j++){                         // loop for row bits    
          if(bitRead(col,j) == 1){                          //if bit j is 1, turn on LED
            digitalWrite(rows[j], rowon);
          }
        }
        delay(1);                                           //waiting, lighting short time    
    ...
    delay(200);                                             // wait for next character
  }  
}
